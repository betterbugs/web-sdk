...
| emailFieldReadOnly          | boolean                                                                                                                                                                                                                                                           | Show the prefilled `email` but stop the reporter from editing it. Applies only while the address from the `email` option or `setEmail()` is a valid one — if it is missing or malformed the field stays editable, so the reporter can supply the address the form requires. Production mode only. Defaults to `false`. |
...
| issueTypes                  | string[]                                                                                                                                                                                                                                                          | Options for the **Issue Type** dropdown in the report form. A list given here replaces the built-in options (`General Enquiry`, `Give Feedback`, `Report a Bug`) instead of adding to them. Entries are trimmed and de-duplicated; if none are usable the SDK warns and keeps the built-in options.|
